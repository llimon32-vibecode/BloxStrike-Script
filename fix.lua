-- =============================================
-- 🍋 NEVERPASTE BS V2 — STABLE
-- by Nevermore / Llimon32
-- Исправлен вылет при инжекте
-- =============================================
...
-- Ждём появления игрока и камеры
...
-- 🗑️ Удаляем старый GUI, если он есть
local oldGui = localPlayer:FindFirstChild("NeverpasteGUI")
...
-- ⚙️ ЛОГИКА (с проверками, чтобы не вылетало)
...
local function updateESP()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        ...
        if visualSettings.showHealth() and char:FindFirstChild("Humanoid") then
...
-- Запускаем ESP с задержкой, чтобы не нагружать
RunService.Heartbeat:Connect(function()
    task.wait(0.1) -- Обновление 10 раз в секунду
    updateESP()
end)
...
-- 🎯 ПЕРЕХВАТ ВЫСТРЕЛОВ (с проверками)
local function hookShoot()
    ...
    if not InventoryController then
        warn("InventoryController not found!")
        return
    end
    local OriginalShoot = InventoryController.ShootWeapon
    if not OriginalShoot then
        warn("ShootWeapon not found!")
        return
    end

    InventoryController.ShootWeapon = function(Self, Data)
        if Data and Data.Bullets and aimSettings.noSpread() then
...
if not success then
    warn("Failed to hook functions: ", err)
end
...
print("🍋 NEVERPASTE BS V2 — STABLE LOADED! Meow ^_^")